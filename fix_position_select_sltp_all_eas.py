#!/usr/bin/env python3
"""Fix PositionSelect SL/TP attachment across Test_Min_Fire_MT5 and all 6 Active EAs."""
...
//|   1-Minute Production Fill & Guaranteed PositionSelect SL/TP    |
//+------------------------------------------------------------------+
...
int OnInit() {
   Print("=== 🧪 TEST MIN FIRE v1.02: PositionSelect SL/TP Engine Active ===");
...
void OpenTestPosition() {
   ...
   if(OrderSend(req, res) && res.retcode == TRADE_RETCODE_DONE) {
      ...
      // Guaranteed Post-Fill PositionSelect SL/TP Attachment
      Sleep(200);
      if(PositionSelect(s)) {
...
bool OpenTrade(int idx, string side, double lot) {
   ...
   // Guaranteed ECN Post-Fill PositionSelect SL/TP Attachment
   Sleep(200);
   if(PositionSelect(s)) {
      ulong pos_tix = PositionGetInteger(POSITION_TICKET);
      if(trade.PositionModify(pos_tix, sl_val, tp_val)) {
...
def patch_all():
    ...
    print("  • Upgraded Test_Min_Fire_MT5.mq5 v1.02 with PositionSelect SL/TP Attachment!")
    ...
    for ea in EAS[1:]:
        ...
        if old_marker in code and "bool ClosePositionByTicket(" in code:
            ...
            ea_code = POSITION_SELECT_OPENTRADE.replace("UltraMonster_entry", f"{ea}_entry")
            code = code[:start_idx] + ea_code + "\n" + code[end_idx:]
            with open(path, "w", encoding="utf-8") as f:
                f.write(code)
            print(f"  • Upgraded {ea}.mq5 with PositionSelect SL/TP Engine!")
...
def push_to_vps():
    print("🚀 Pushing all updated PositionSelect SL/TP EAs to VPS...")
    for ea in EAS:
...
def main():
    print("="*115)
    print("APPLYING GUARANTEED POSITIONSELECT SL/TP ENGINE ACROSS ALL EAS...")
    print("="*115)
    patch_all()
    ...
    print("="*115)
    print("🟢 ALL EAS SUCCESSFULLY UPGRADED WITH POSITIONSELECT SL/TP ENGINE & PUSHED TO VPS!")
    print("="*115)
