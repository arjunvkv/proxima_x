...
- **#21 NOTE (corrected 2026-08-11):** the first probe used the WRONG index sign (DXY weights EURUSD at -0.576, probe used +0.576) - its residual was contaminated with EURUSD's own drift. Corrected residual r = log(DXY) + 0.576*log(EURUSD) is CLEANER and STRONGER: control (plain EURUSD 20d z reversion, no DXY) LOSES -$75/lot, proving the edge lives in the DXY-vs-EURUSD gap, not EURUSD self-reversion.
- **#21 DEEP BATTERY (battery_dxy.py): ALL GREEN - deployable candidate "S4".** n=33, win 78.8%, exp +$370/lot, PF 11.79; LODO 33/33; monthly POSITIVE ALL 7 MONTHS ($76-$519); plateau 9/9 (window 10/20/40 x thr 1.0/1.5/2.0); regime halves both positive (exp $504 H1 / $272 H2, decaying); 2x spread stress net $1,833->$1,803 (insensitive); MC1000 daily-shuffle @1.86 lots mean maxDD $659, p95 $1,079, 0% breach >$10k; FTMO sim 1.86 lots worst 2x-day exactly $5,000, maxDD $664, loss streak 2d, +$22,783 total (33.2%/mo); entry +1-day robust (exp $345 vs $370). Caveats: 7-month envelope only (unseen-regime pull pending), n=33 at chosen threshold, H2 decay, daily-cadence signal needs NOVA live mode (P3).
