"""
DPL-7.2: TCMA + Structured Readout Layer (SRL)
  - TCMA: temporal contrastive alignment of latent space
  - SRL: multi-directional gated subspace readout
  - Metrics: IC, DA, LSS, TDI, PnL attribution
"""
...
from research.directional_physics.dpl7.policy.signal_head import StructuredReadoutLayer
...
def run(horizons=None, symbols=None):
    ...
    print("=" * 80)
    print("  DPL-7.2 WALK-FORWARD: TCMA + Structured Readout Layer")
    print("=" * 80)
    ...
    for horizon in horizons:
        print(f"\n{'=' * 60}")
        print(f"  H{horizon} WALK-FORWARD (TCMA + SRL)")
        print(f"{'=' * 60}")
        ...
        for split_idx in range(len(years) - 3):
            ...
            # Step 1: TCMA pretraining
            tcma = TemporalContrastiveAligner(dim=dpl7_config.LATENT_DIM, tau=0.5, lambda_drift=0.1)
            losses = tcma.pretrain(z_train_normed, train_records, n_epochs=3, lr=0.001)

            # Apply TCMA alignment
            ...
            # Step 2: SRL training on aligned space
            srl = StructuredReadoutLayer(dim=dpl7_config.LATENT_DIM, k=4)
            train_ic = srl.fit(al_train, ret_train, lr=0.001, epochs=5)

            # Evaluate on test
            test_scores = np.array([srl.forward(z)["score"] for z in al_test])
            test_dir = np.tanh(test_scores)
            test_conf = np.array([srl.forward(z)["confidence"] for z in al_test])

            train_scores = np.array([srl.forward(z)["score"] for z in al_train])
            ...
            print(f"    LSS: {test_lss:.4f} ({lss_str}) | TDI: {test_tdi:.4f} | Drift: {drift:.4f}")
        ...
        if split_results:
            ...
            print(f"    IC survival (>0.02): {n_positive_ic}/{n_splits} = {round(n_positive_ic/n_splits*100,1)}%  (mean={np.mean(test_ics):.4f})")
            print(f"    DA survival (>52%):  {n_positive_da}/{n_splits} = {round(n_positive_da/n_splits*100,1)}%  (mean={np.mean(test_das):.4f})")
            print(f"    PnL improve (>0):    {n_positive_pnl}/{n_splits} = {round(n_positive_pnl/n_splits*100,1)}%")
            print(f"    LSS stable (>0.80):  {n_stable_lss}/{n_splits} = {round(n_stable_lss/n_splits*100,1)}%  (mean={np.mean(test_lsss):.4f})")
            print(f"    Mean TDI: {np.mean(test_tdis):.4f} | Mean drift: {np.mean(drifts):.4f} | Max IC: {max(test_ics):.4f}")

            results[f"H{horizon}"] = {
                "n_splits": n_splits,
                "ic_survival_pct": round(n_positive_ic / n_splits * 100, 1),
                "da_survival_pct": round(n_positive_da / n_splits * 100, 1),
                "pnl_survival_pct": round(n_positive_pnl / n_splits * 100, 1),
                "lss_survival_pct": round(n_stable_lss / n_splits * 100, 1),
...
def main():
    ...
    print(f"\n{'=' * 80}")
    print("  DPL-7.2 AGGREGATE FINDINGS")
    print("=" * 80)
    ...
    ic_ok = sum(1 for h in results.values() if h["mean_test_ic"] > 0.03)
    da_ok = sum(1 for h in results.values() if h["mean_test_da"] > 0.54)
    ...
    if ic_ok >= n_h - 1 and da_ok >= n_h - 1 and pnl_ok >= n_h // 2:
        print("  VERDICT: POSITIVE EDGE CONFIRMED (stable representation + directional extraction + PnL)")
    elif ic_ok >= n_h // 2 or da_ok >= n_h // 2:
        print("  VERDICT: MARGINAL (partial signal, needs stronger extraction)")
    else:
        print("  VERDICT: NO EDGE (TCMA+SRL pipeline does not extract profitable signal)")

    out_dir = os.path.join(os.path.dirname(__file__), "..", "..", "reports")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "dpl7_2_walkforward_results.json")
