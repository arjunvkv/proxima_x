"""
Idea 2: z-Extreme + ATR Gate — M1 trailing stop on high-conviction bars only.
Filters: |z| > threshold AND bar ATR > rolling median ATR (skip low-vol bars).
"""
...
def run(pair, cost, z_thresh, atr_percentile):
    t = load(pair)
    ...
    # ATR gate: only trade bars where ATR > percentile threshold of recent 100 bars
    atr_gate = atr.rolling(100, min_periods=10).quantile(atr_percentile).bfill()
    ...
    return {'n': len(pnls), 'tpd': tpd, 'wr': wr, 'avg': avg, 'net': net,
            'win_avg': win_avg, 'loss_avg': loss_avg, 'payoff': payoff,
            'days': len(ndays)}

print(f"{'Pair':>7s}  {'z_thr':>5s}  {'ATRp':>5s}  {'tpd':>6s}  {'WR':>5s}  {'avg':>9s}  {'net':>9s}  {'payoff':>6s}  {'n':>6s}")
print("-" * 75)
t0 = time.time()
for pair in ['EURUSD', 'EURJPY', 'GBPJPY']:
    for zt in [0.0, 0.5, 1.0, 1.25, 1.5, 2.0]:
        for atrp in [0.0, 0.25, 0.50]:
            r = run(pair, COST[pair], zt, atrp)
            if r:
                print(f"{pair:>7s}  {zt:>5.2f}  {atrp:>5.0%}  {r['tpd']:>6.0f}  {r['wr']:>5.1%}  {r['avg']:>+9.2f}  {r['net']:>+9.2f}  {r['payoff']:>6.1f}  {r['n']:>6d}")
