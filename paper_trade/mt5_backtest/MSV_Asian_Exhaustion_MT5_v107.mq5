...
//|   🔥 MSV Asian Exhaustion v107 — 18-Pair Asian FX Network Exhaustion|
//|   v107: Magic Base 202650 (Zero Collision), SL=50p/TP=80p Guard  |
//+------------------------------------------------------------------+
...
input double   BASE_LOT            = 0.18;    // Base Lot Size
input int      HOLD_BARS           = 12;      // 60m Hold Time (12 M5 bars)
input ulong    MAGIC_BASE          = 202650;  // Magic Base (Non-overlapping)
...
int OnInit() {
   Print("=== MSV Asian Exhaustion MT5 Init v107 MagicBase=", MAGIC_BASE, " ===");
...
bool OpenTrade(int idx, string side, double lot) {
   ...
   req.magic = MAGIC_BASE + idx; req.comment = "MSV_Asian_v107";
   ...
   Print("  🟢 ENTRY MSV_Asian ", s, " ", side, " v=", req.volume, " at=", res.price, " SL=", sl_price, " TP=", tp_price);
   return true;
}
...
bool CloseTrade(int idx, string why) {
   ...
   req.deviation = (ulong)(SLIPPAGE_PIPS * 10);
   req.magic = MAGIC_BASE + idx; req.comment = "MSV_Asian_exit";
   
   if(!OrderSend(req, res) || res.retcode != TRADE_RETCODE_DONE) return false;
   
   double pnl = PositionGetDouble(POSITION_PROFIT);
   Print("  🔴 CLOSE MSV_Asian ", s, " ", why, " pnl=", pnl);
...
void CheckEntry() {
   ...
   for(int i=0; i<N_PAIRS; i++) {
      ...
      if(CopyRates(s, PERIOD_M5, 1, 12, rates) < 12) continue;
