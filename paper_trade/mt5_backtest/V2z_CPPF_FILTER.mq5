...
#property description "V2z CPPF FILTER - RECON + spread/ATR entry gate"
...
input double   SPREAD_ATR_THRESHOLD = 0.08;
...
int OnInit() {
   ...
   ClearPosition();
   Print("V2z_CPPF_FILTER on ", _Symbol, " magic=", MAGIC_NUMBER, " thr=", SPREAD_ATR_THRESHOLD);
...
bool OpenPosition(int direction, double atr_v) {
   double spread_ratio = GetSpreadPips() * _Point / atr_v;
...
void CheckEntry() {
   ...
   if(MathAbs(z) >= Z_THRESHOLD) {
      double spread_atr_ratio = GetSpreadPips() * _Point / av;
      if(spread_atr_ratio > SPREAD_ATR_THRESHOLD) {
         Print("SKIP ", _Symbol, " z=", z, " atr=", av, " sprd=", GetSpreadPips(),
               " satr=", spread_atr_ratio);
         return;
      }
      int dir = (z > 0) ? -1 : 1;
      OpenPosition(dir, av);
