//+------------------------------------------------------------------+
//|                                NY_H21_MT5_v107.mq5               |
//|   🔥 NY H21 Engine v107 — 21:00 UTC Closing Bell Mean Reversion  |
//|   v107: Magic Base 202670 (Zero Collision), SL=50p/TP=80p Guard  |
//+------------------------------------------------------------------+
...
input double   BASE_LOT            = 0.25;    // Base Lot Size
input int      HOLD_BARS           = 12;      // 60m Hold Time (12 M5 bars)
input int      LOOKBACK_BARS       = 12;      // 60m Lookback
input int      SESSION_HOUR        = 21;      // 21:00 UTC Session (gated on TimeGMT)
input ulong    MAGIC_BASE          = 202670;  // Magic Base (Non-overlapping)
...
#define N_PAIRS 2
string PAIRS[N_PAIRS] = {"EURJPY", "GBPJPY"};
...
int OnInit() {
   Print("=== NY H21 Engine MT5 Init v107 (21:00 UTC Close Reversion) MagicBase=", MAGIC_BASE, " ===");
...
bool OpenTrade(int idx, string side, double lot) {
   ...
   req.magic = MAGIC_BASE + idx; req.comment = "NY_H21_v107";
   ...
   Print("  🟢 ENTRY NY_H21 ", s, " ", side, " v=", req.volume, " at=", res.price, " SL=", sl_price, " TP=", tp_price);
   return true;
}
...
bool CloseTrade(int idx, string why) {
   ...
   req.deviation = (ulong)(SLIPPAGE_PIPS * 10);
   req.magic = MAGIC_BASE + idx; req.comment = "NY_H21_exit";
   
   if(!OrderSend(req, res) || res.retcode != TRADE_RETCODE_DONE) return false;
   
   double pnl = PositionGetDouble(POSITION_PROFIT);
   Print("  🔴 CLOSE NY_H21 ", s, " ", why, " pnl=", pnl);
...
void CheckEntry() {
   ...
   for(int i=0; i<N_PAIRS; i++) {
      ...
      if(CopyRates(s, PERIOD_M5, 1, LOOKBACK_BARS, rates) < LOOKBACK_BARS) continue;
      
      double c_now = rates[0].close;
      double c_prev = rates[LOOKBACK_BARS-1].close;
