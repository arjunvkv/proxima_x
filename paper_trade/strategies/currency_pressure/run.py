"""Run Currency Inventory Pressure paper trading."""
...
def main():
    ...
    risk = Risk(cfg)
    exec_ = Executor(feed, logger, magic=cfg.get("magic", 202403))
    dash = Dashboard()
    ...
    warmup_seconds = 120  # quick trial: 2 min warmup
    ...
    try:
        while True:
            ...
            # Warm-up: collect data but don't trade yet
            elapsed = time.time() - warmup_start
            if elapsed < warmup_seconds:
                generate_signal(data, current_time=current_ts)
                time.sleep(0.5)
                continue
            if warmup_seconds > 0:
                print(f"Warmup complete ({elapsed:.0f}s). Trading active.", file=sys.stderr)
                warmup_seconds = 0

            signals = generate_signal(data, current_time=current_ts)
            if signals:
                for signal in signals:
                    ...
                    if not ok:
                        ...
                        fill = exec_.submit_market(pair, direction, cfg["lot_size"], signal_meta=signal.get("metadata"))
                        if fill:
                            stats.record_fill(fill)
                            dash.set_last_signal(f"{pair}{'+' if direction>0 else '-'}")

            closed = exec_.check_open_positions(current_ts, hold_seconds=cfg["hold_minutes"] * 60)
            for c in closed:
