"""Run paper trading for Combined DC + 10sMR strategy."""
...
def _close_expired(executor, now):
    """Close positions past their individual hold_seconds."""
    closed = []
    remaining = []
    for pos in executor.positions:
        age = now - pos["entry_time"]
        hold = pos.get("hold_seconds", CONFIG.get("dc_hold_seconds", 600))
        if age >= hold:
            result = executor.close_position(pos, exit_time=now)
            if result:
...
def main():
    ...
    seed_history(feed)
    risk = Risk(cfg)
    executor = Executor(feed, logger, magic=cfg.get("magic", 202409))
    dash = Dashboard()
    ...
    try:
        while True:
            ...
            if signals is not None:
                for signal in signals:
                    if signal.get("confidence", 0) >= MIN_CONFIDENCE:
                        ...
                        ok_s, reason = risk.check_all(
                            now_ts, cur_spread, norm_spread,
                            len(executor.positions), pair=pair
                        )
                        if not ok_s:
                            logger.log("BLOCK", pair, f"{signal['signal_type']}:{reason}")
                            stats.record_reject(pair, reason)
                        else:
                            signal_meta = {
                                "z_score": signal["metadata"].get("z"),
                                "signal_type": signal["signal_type"],
                            }
                            fill = executor.submit_market(
                                pair, direction, cfg["lot_size"],
                                signal_meta=signal_meta,
                            )
                            if fill:
                                fill["hold_seconds"] = signal.get(
                                    "hold_seconds",
                                    cfg.get("dc_hold_seconds", 600),
                                )
            ...
            closed = _close_expired(executor, now_ts)
            for c in closed:
