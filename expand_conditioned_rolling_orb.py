#!/usr/bin/env python3
"""Quantitative Trade Frequency Expansion for Conditioned Rolling Hourly ORB."""
import sys, time
...
def run_expanded_orb(df_all, close_mat, open_mat, high_mat, low_mat, hours, minutes, pairs_subset, min_range_pips=12.0, allowed_hours=range(6, 19), trigger_mins=[0], hold_bars=3):
...
def main():
    t0 = time.time()
    print("Loading M5 dataset for Conditioned Rolling ORB Frequency Expansion...")
    ...
    print("\n" + "="*95)
    print("FREQUENCY EXPANSION AUDIT: MAINTAINING >= 75.0% WIN RATE")
    print("="*95)

    pair_sets = [
        ("4 Core Crosses", ["GBPAUD", "GBPNZD", "EURAUD", "GBPJPY"]),
        ("6 Top Pairs (+EURJPY, USDJPY)", ["GBPAUD", "GBPNZD", "EURAUD", "GBPJPY", "EURJPY", "USDJPY"]),
        ("7 Top Pairs (+GBPUSD)", ["GBPAUD", "GBPNZD", "EURAUD", "GBPJPY", "EURJPY", "USDJPY", "GBPUSD"]),
        ("All 9 Pairs", PAIRS_ALL)
    ]

    sweep_rows = []
    for p_name, p_list in pair_sets:
        for min_p in [10.0, 12.0, 15.0]:
            for trig_m in [[0], [0, 30]]:
                df_e = run_expanded_orb(df_all, close_mat, open_mat, high_mat, low_mat, hours, minutes, p_list, min_p, range(6, 20), trig_m, 3)
                if df_e.empty: continue
                pnls = df_e["net_pnl"].values
                n_t = len(pnls)
                ...
                if wr >= 75.0:
                    sweep_rows.append({
                        ...
                        "PF": round(pf, 2),
                        "Max DD": f"${max_dd:.2f}",
                        "Status": "🟢 HIGH FREQ CHAMPION" if trades_per_day >= 10.0 else "PASS"
                    })
