#!/usr/bin/env python3
"""MSV Asian FX Currency Network Dispersion — High Quality Filter Sweep."""
import sys, time
...
# JPY & Commodity High-Beta Pairs (highest Asian mean-reversion quality)
HIGH_QUALITY_PAIRS = ["AUDJPY", "EURJPY", "GBPJPY", "USDJPY", "EURAUD", "GBPAUD", "AUDUSD", "NZDUSD"]
...
def main():
    ...
    print("\n" + "="*85)
    print("MSV ASIAN EXHAUSTION — HIGH-QUALITY FILTER SWEEP (FUNDEDNEXT)")
    print("="*85)

    sim = ExecutionSimulator("fundednext")

    configs = [
        (0.95, -0.0002, ALL_PAIRS, "Base Config (Disp >= 95%, Dec <= -0.02%, All Pairs)"),
        (0.97, -0.0003, ALL_PAIRS, "Filter 1 (Disp >= 97%, Dec <= -0.03%, All Pairs)"),
        (0.98, -0.0004, ALL_PAIRS, "Filter 2 (Disp >= 98%, Dec <= -0.04%, All Pairs)"),
        (0.97, -0.0003, HIGH_QUALITY_PAIRS, "Filter 3 (Disp >= 97%, Dec <= -0.03%, JPY/AUD Basket Only)"),
        (0.98, -0.0004, HIGH_QUALITY_PAIRS, "Filter 4 (Disp >= 98%, Dec <= -0.04%, JPY/AUD Basket Only)"),
    ]

    for pct_thresh, dec_thresh, pair_universe, name in configs:
        pair_indices = [ALL_PAIRS.index(p) for p in pair_universe]
        n_bars = len(df_all)
        in_basket = False
        exit_bar = 0
        ...
        for t in range(505, n_bars):
            ...
            if not in_basket and 0 <= hours[t] <= 6:
                ...
                if mean_r <= dec_thresh and disp_pct[t] >= pct_thresh:
                    basket_pairs = [p_i for p_i in pair_indices if r_t[p_i] < mean_r]
                    if len(basket_pairs) >= 2:
                        in_basket = True
                        exit_bar = t + 6  # 30-min hold
                        for p_i in basket_pairs:
        ...
        print(f"\n--- {name} ---")
        print(f"  Basket Events : {len(basket_pnls)}")
