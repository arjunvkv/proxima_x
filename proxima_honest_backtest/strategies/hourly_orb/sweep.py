#!/usr/bin/env python3
"""#4 Rolling Hourly ORB — sweep + validation."""
import sys, time, json, math
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent.parent.parent))
import pandas as pd
import numpy as np
from proxima_honest_backtest.strategies.hourly_orb.strategy import RollingORBStrategy, ALL_PAIRS
...
def run_cfg(data, pre_align, hold, min_pip, broker):
    s = RollingORBStrategy({"hold_bars": hold, "min_breakout_pips": min_pip})
...
def main():
    ...
    print(f"\n{'='*70}")
    print("PHASE 1: Rolling Hourly ORB sweep on Exness")
    print(f"{'='*70}")
    phase1 = []
    t1 = time.time()
