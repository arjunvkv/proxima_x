#!/usr/bin/env python3
"""#5 Intraday Seasonality — sweep + validation."""
import sys, time, json, math
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent.parent.parent))
import pandas as pd
import numpy as np
from proxima_honest_backtest.strategies.seasonality.strategy import SeasonalityStrategy, ALL_PAIRS
...
def run_cfg(data, pre_align, top_n, hold, broker):
    s = SeasonalityStrategy({"top_n": top_n, "hold_bars": hold})
...
def main():
    ...
    print(f"\n{'='*70}")
    print("PHASE 1: Seasonality sweep on Exness")
    print(f"{'='*70}")
    phase1 = []
    t1 = time.time()
    total = len(TOPN) * len(HOLDS)
    cfg_idx = 0
    for top_n in TOPN:
        for hold in HOLDS:
            cfg_idx += 1
            r = run_cfg(raw, pre_align, top_n, hold, "exness")
            et = time.time() - t1; t1 = time.time()
            e = {"top_n": top_n, "hold": hold,
                 "trades": r.n_trades, "net_pnl": r.net_pnl,
                 "wr": r.win_rate, "pf": r.profit_factor}
            phase1.append(e)
            print(f"  [{cfg_idx:>2d}/{total}] n={top_n:>2d} hold={hold:>2d} | "
    ...
    for i, r in enumerate(phase1):
        print(f"  #{i+1} n={r['top_n']:>2d} hold={r['hold']:>2d} | "
    ...
    for broker in BROKERS:
        r = run_cfg(raw, pre_align, best["top_n"], best["hold"], broker)
        e = {**{k: best[k] for k in ["top_n","hold"]},
    ...
    print(f"{'='*70}")
    base = run_cfg(raw, pre_align, best["top_n"], best["hold"], "exness")
