#!/usr/bin/env python3
"""Run MT5-Equivalent Execution Simulation for Filter 4 Optimal."""
import sys, time
...
def main():
    ...
    print("\n" + "="*85)
    print("MT5 FUNDEDNEXT SERVER 3 — MSV FILTER 4 OPTIMAL BACKTEST RESULTS")
    print("="*85)
    ...
    for t in range(505, n_bars):
        if in_basket and t >= exit_bar:
            ...
            for p_i in basket_pairs:
                ...
                # 0.15 Lot per pair
                pnl = (c_exit - c_entry) / c_entry * 15000.0
            ...
            trade_logs.append({
                "Date": str(times[t]),
                "Basket_Size": len(basket_pairs),
                "Gross_PnL": round(gross_basket_pnl, 2),
                "Commission": round(total_comm, 2),
                "Net_PnL": round(net_basket_pnl, 2),
                "Result": "WIN" if net_basket_pnl > 0 else "LOSS"
            })
            in_basket = False
    ...
    print(f"Total Basket Events : {len(basket_pnls)}")
    print(f"Winning Events      : {len(wins)} ({wr:.1f}%)")
    print(f"Losing Events       : {len(losses)} ({100-wr:.1f}%)")
    print(f"Gross PnL           : +${gw - gl + sum(t['Commission'] for t in trade_logs):.2f}")
    print(f"Total Commissions   : -${sum(t['Commission'] for t in trade_logs):.2f}")
    print(f"Net PnL             : +${net:.2f}")
    print(f"Profit Factor       : {pf:.2f}")
    print(f"Avg PnL / Event     : +${avg_b:.2f}")
    print("="*85)

    df_logs = pd.DataFrame(trade_logs)
    print("\nRecent 10 Basket Trade Logs:")
    print(df_logs.tail(10).to_string(index=False))
