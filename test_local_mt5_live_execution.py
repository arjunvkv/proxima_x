#!/usr/bin/env python3
"""Local MT5 Live Execution Test: Test volume normalization, SL/TP attachment, and position close via MetaTrader5 Python API."""
import time
import MetaTrader5 as mt5

def main():
    print("="*115)
    print("LOCAL MT5 LIVE EXECUTION TEST (VOLUME NORMALIZATION & ECN SL/TP ATTACHMENT)")
    print("="*115)
    ...
    print(f"  • Connected Account: #{acc_info.login} ({acc_info.company})")
    ...
    # 1. Step Volume Normalization
    raw_lot = 1.20
    step = sym_info.volume_step
    min_vol = sym_info.volume_min
    max_vol = sym_info.volume_max
    
    normalized_vol = round(round(raw_lot / step) * step, 2)
    normalized_vol = max(min_vol, min(max_vol, normalized_vol))

    print(f"  1. VOLUME NORMALIZATION CHECK:")
    print(f"     Raw Volume Input: {raw_lot} L")
    print(f"     Symbol Volume Step: {step} | Min: {min_vol} | Max: {max_vol}")
    print(f"     Normalized Volume Output: {normalized_vol:.2f} L 🟢")
    print("="*115)
    ...
    print(f"  2. PLACING MARKET BUY ORDER ON {symbol}:")
    print(f"     Ask Price: {price:.5f}")
    print(f"     Target SL: {sl_target:.5f} (-35.0 pips)")
    print(f"     Target TP: {tp_target:.5f} (+45.0 pips)")
    ...
    if res_entry is None or res_entry.retcode != mt5.TRADE_RETCODE_DONE:
        print(f"⚠️ Initial OrderSend IOC failed: {res_entry.comment if res_entry else mt5.last_error()}. Retrying with FOK...")
        req_entry["type_filling"] = mt5.ORDER_FILLING_FOK
        res_entry = mt5.order_send(req_entry)

    if res_entry is None or res_entry.retcode != mt5.TRADE_RETCODE_DONE:
        print(f"❌ Market BUY Order failed! Retcode: {res_entry.retcode if res_entry else 'None'}, Comment: {res_entry.comment if res_entry else ''}")
        mt5.shutdown()
        return

    order_ticket = res_entry.order
    fill_price = res_entry.price
    print(f"  🟢 MARKET BUY ORDER FILLED SUCCESSFULLY!")
    ...
    print(f"  3. ECN POST-FILL POSITIONMODIFY SL/TP ATTACHMENT:")
    ...
    # 4. Verify Position Status in MT5 Broker Memory
    time.sleep(0.5)
    pos_verify_list = mt5.positions_get(ticket=pos_ticket)
    if pos_verify_list:
        p_ver = pos_verify_list[0]
        print("="*115)
        print("  4. LIVE VERIFICATION OF ATTACHED POSITION IN MT5 BROKER MEMORY:")
        print(f"     Symbol: {p_ver.symbol}")
    ...
    # 5. Clean Position Close Test
    time.sleep(1.0)
    print(f"  5. TESTING CLEAN POSITION CLOSE VIA TRADE_ACTION_DEAL:")
    tick_close = mt5.symbol_info_tick(symbol)
    req_close = {
        "action": mt5.TRADE_ACTION_DEAL,
        "symbol": symbol,
        "volume": float(p_ver.volume),
        "type": mt5.ORDER_TYPE_SELL,
        "position": pos_ticket,
        "price": tick_close.bid,
        "deviation": 10,
        "magic": 999999,
        "comment": "Local_MT5_Test_Close",
        "type_time": mt5.ORDER_TIME_GTC,
        "type_filling": mt5.ORDER_FILLING_IOC,
    }
    ...
    if res_close is not None and res_close.retcode == mt5.TRADE_RETCODE_DONE:
        print(f"  🟢 POSITION CLOSED CLEANLY AT {res_close.price:.5f}!")
        print(f"     Close Deal Ticket: #{res_close.deal}")
    else:
        print(f"  ❌ Close Position Failed: {res_close.comment if res_close else mt5.last_error()}")

    print("="*115)
    print("🟢 LOCAL MT5 LIVE EXECUTION TEST COMPLETED WITH 100% SUCCESS!")
    print("="*115)
    mt5.shutdown()
